...
def handle_keyboard_input(key_press_queue):
    def on_press(key):
        if key == keyboard.Key.enter:
            key_press_queue.put("enter")
        if key == keyboard.Key.tab:
            key_press_queue.put("tab")
...
def play_audio_and_run_code(questions,arg):
    voice_list = generate_voice_order()
    with open(f'{arg}-{voice_list[0]}-full_script.txt', 'w') as file:
        file.write(voice_list[0])
        file.write('\n')

    flag = False
    for _, value in questions.items():
        for question in value:
            pr("Press enter to play the next audio file. Press tab to stop playing audio")
            if key_press_queue.get() == 'enter':
                retrieve_sentence(voice_list[0], question)
            else:
                pr('Stopping audio...')
                flag = True
                break
...
if __name__ == '__main__':
    ...
    pr("Starting...")
    pr("Press enter to paly next audio or press esc to quit")

    arg = sys.argv[1]
    keyboard_input_thread = threading.Thread(target=handle_keyboard_input, args=(key_press_queue,))

    keyboard_input_thread.start()
    play_audio_and_run_code(questions,arg)
